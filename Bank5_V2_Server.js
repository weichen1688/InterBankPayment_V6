...
var Bank5Contract = web3.eth.contract(contractAbi.BankAbi);
var Bank5 = Bank5Contract.at(contractAbi.Bank5ContractAddress);

var Bank5_admin = '0x0638e1574728b6d862dd5d3a3e0942c3be47d996';
...
if(web3.isConnected()) {
    ...
    console.log('connected to quorum enode [node5:22004], starting event listening ...');
    ...
    event1.watch(function(error, event){
        if (error) {
            ...
            console.log(esc_red_str+"交易序號："+ret0+" EventForSecuritiesTransactionWaitingForPayment"+esc_red_end + " txHash " + txhash + " " + " Block Number " + blocknum );
            console.log(esc_red_str+"交易序號："+ret1+" EventForSecuritiesTransactionWaitingForPayment"+esc_red_end + " txHash " + txhash + " " + " Block Number " + blocknum );
            ...
            if(Process_Mode == "EZ") {
                ...
                if (frombank != "Bank5") {
                    ...
                    var result = dbConnection.collection("paymentsecret").find({"txnseq": seq}).toArray(function (err, result) {
                        ...
                        } else if (tobank == "Bank2") {

                            var req = 'http://localhost:3001/sendToBuyer/' + s0 + '/' + s1 + '/' + _payment_hash + '/' + _payment_secret;
    ...
    event2.watch(function(error, event){
        if (error) {
            ...
            console.log(esc_red_str+"交易序號："+s0+" "+s1+" EventForSecuritiesTransactionFinished"+esc_red_end + " txHash " + txhash + " " + " Block Number " + blocknum );
    ...
    event3.watch(function(error, event){
        if (error) {
            ...
            console.log(esc_red_str+"交易序號："+s0+" "+s1+" EventForSecuritiesTransactionPaymentError"+esc_red_end + " txHash " + txhash + " " + " Block Number " + blocknum );
    ...
    event4.watch(function (err, event) {
        if (!err) {
            ...
            console.log(esc_red_str+"交易序號："+s0+" EventForSecuritiesTransactionCancelled"+esc_red_end + " txHash " + txhash + " " + " Block Number " + blocknum );
            ...
            if(retA == "3") {
                ...
                if(frombank != tobank && frombank == "Bank5") {   //不是賣方節點，不做
    ...
    event5.watch(function (err, event) {
        if (!err) {
            ...
            console.log(esc_red_str+"交易序號："+ s0 + " submitInterBankTransaction ，is Pending" + esc_red_end + " txHash " + txhash + " " + " Block Number " + blocknum );
    ...
    console.log('connect to enode [node2:22004] failed');
    return;
}
...
server = Http.createServer( function( request, response ) {
    router( request, response, function( error ) {
        ...
        response.end( 'Bank5 RESTful API is running!' );
    });
});
...
router.get( '/purchaseBond/Bank5/:type/:frombank/:fromcustomer/:tobank/:tocustomer/:amount/:securities_id/:payment/:rev_txn/:rev_reason', function (request, response) {
    ...
    var txnseq = "Bank5"+ type+ fromcustomer+ ts + nonce;
    ...
    console.log("purchaseBond from " + frombank + " " + fromcustomer + " to " + tobank + " " + tocustomer + " amount " +amount +
    ...
    if(Process_Mode == "EZ") {
        ...
        if ( cash_payment > cash_pos && tobank == "Bank5" ) {
...
router.get( '/purchaseBond_N/Bank5/:type/:frombank/:fromcustomer/:tobank/:tocustomer/:amount/:securities_id/:payment/:rev_txn/:rev_reason', function (request, response) {
...
router.get( '/checkAccBal/:securities_id/:bank_id/:cust_id', function (request, response) {
    ...
    var fields2 = Bank5.getCustomerInfo(cust_id);
...
router.get( '/checkBanner/:securities_id/:bank_id/:cust_id', function (request, response) {
    ...
    var fields = Bank5.getCustomerSecuritiesInterest(_sec_id, _cust_id);
    ...
    var data3 = Bank5.getLastPaidInterest(_sec_id, _cust_id);
    var last_interest = JSON.stringify(data3);

    var _block = web3.eth.blockNumber;
    var _nonce = web3.eth.getTransactionCount(Bank5_admin);
...
router.get( '/unlockAccount/:passwd/:period', function (request, response) {
    ...
    var isUnlocked = web3.personal.unlockAccount(Bank5_admin, _passwd, _period);
...
router.get( '/lockAccount', function (request, response) {
    web3.personal.lockAccount(Bank5_admin);
...
router.get( '/cancelTransaction/:frombank/:tobank/:txn', function (request, response) {
    ...
    if(_from_bank=="Bank5" && _to_bank == "Bank5") {
...
router.get( '/setCustomerInfo/Bank5/:cust_id/:cust_name/:cust_type', function (request, response) {
    ...
    Bank5.setCustomerInfo(_cust_id, _cust_name, _cust_type, {
        from: Bank5_admin,
        gas: 6000000,
        privateFor: [
            key_Bank1,
        ...
        if (!err) {
            ...
            var theEvent = Bank5.EventForSetCustomerInfo({
                from: Bank5_admin,
            });
...
function autotest2() {
    ...
    //var type = TEST_TYPE; // BUY or SELL
    var frombank = "Bank5";
    var fromcustomer = "005-000-00001";
    var tobank = "Bank5";
    var tocustomer = "005-000-00002";
    var amount = TEST_AMOUNT;
    var securities_id = "A07101";
    var payment = TEST_AMOUNT;
    ...
    var nonce = Math.floor((Math.random() * 100));
    var ts = Math.round(new Date().getTime()/1000);
    var txnseq_1 = "Bank5S"+ fromcustomer+ ts + nonce;
    var txnseq_2 = "Bank5B"+ fromcustomer+ ts + nonce;
