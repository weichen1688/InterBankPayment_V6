...
MongoClient.connect(MongoDB_URI, function (err, db) {
    if (err) throw err;
    dbConnection = db.db("bank4");
    console.log("mongodb connected")
});

var Web3 = require("web3");
var web3 = new Web3(new Web3.providers.HttpProvider('http://localhost:22003'));
var eth = web3.eth;
...
var Bank4Contract = web3.eth.contract(contractAbi.BankAbi);
var Bank4 = Bank4Contract.at(contractAbi.Bank4ContractAddress);

var Bank4_admin = '0x9186eb3d20cbd1f5f992a950d808c4495153abd5';
...
if(web3.isConnected()) {
    ...
    console.log('connected to quorum enode [node4:22003], starting event listening ...');
    ...
    event4.watch(function (err, event) {
        if (!err) {
            ...
            if(retA == "3") {
                ...
                if(frombank != tobank && frombank == "Bank4") {   //不是賣方節點，不做
    ...
    console.log('connect to enode [node4:22003] failed ');
    return;
}
...
server = Http.createServer( function( request, response ) {
    router( request, response, function( error ) {
        ...
        response.end( 'Bank4 RESTful API is running!' );
    });
});
...
router.get( '/purchaseBond/Bank4/:type/:frombank/:fromcustomer/:tobank/:tocustomer/:amount/:securities_id/:payment/:rev_txn/:rev_reason', function (request, response) {
    ...
    var txnseq = "Bank4"+ type+ fromcustomer+ ts + nonce;
    ...
    if(Process_Mode == "EZ") {
        ...
        if ( cash_payment > cash_pos && tobank == "Bank4" ) {
    ...
    var txHashA = TransactionMatcher.submitInterBankTransaction(txnseq, frombank, fromcustomer, tobank, tocustomer, amount, securities_id, payment, md5_digest, {
            ...
            if (!err) {
                ...
                if(Process_Mode == "EZ") {
                    ...
                    console.log("submitInterBankPayment "+ payment_md5_hash + " " + CC_TIMEOUT);
...
router.get( '/purchaseBond_N/Bank4/:type/:frombank/:fromcustomer/:tobank/:tocustomer/:amount/:securities_id/:payment/:rev_txn/:rev_reason', function (request, response) {
    ...
    var position = TransactionMatcher.getCustomerSecuritiesPosition(securities_id, frombank, fromcustomer);
    if (amount > position && type == 'S') {  // 賣方才檢查

        console.log("賣方面額不足"+amount+" "+position);
...
router.get( '/checkTxnInfo/Bank2/:txnseq', function (request, response) {
    ...
    console.log("/checkTxnInfo/Bank4/:checkTxnInfo " + txnseq);
    ...
    //enum TxnState { Pending, Matched, Finished, Cancelled, PaymentError, Waiting4Payment }
...
router.get( '/checkAccBal/:securities_id/:bank_id/:cust_id', function (request, response) {
    ...
    var fields2 = Bank4.getCustomerInfo(cust_id);
...
router.get( '/checkBanner/:securities_id/:bank_id/:cust_id', function (request, response) {
    ...
    var fields = Bank4.getCustomerSecuritiesInterest(_sec_id, _cust_id);
    ...
    var data3 = Bank4.getLastPaidInterest(_sec_id, _cust_id);
    var last_interest = JSON.stringify(data3);

    var _block = web3.eth.blockNumber;
    var _nonce = web3.eth.getTransactionCount(Bank4_admin);
...
router.get( '/unlockAccount/:passwd/:period', function (request, response) {
    ...
    var isUnlocked = web3.personal.unlockAccount(Bank4_admin, _passwd, _period);
...
router.get( '/lockAccount', function (request, response) {
    web3.personal.lockAccount(Bank4_admin);
...
router.get( '/cancelTransaction/:frombank/:tobank/:txn', function (request, response) {
    ...
    console.log("/cancelTransaction/" + txn + "/" + _from_bank + "/" + _to_bank + "/" + participant);


    if(_from_bank=="Bank4" && _to_bank == "Bank4") {
...
router.get( '/setCustomerInfo/Bank4/:cust_id/:cust_name/:cust_type', function (request, response) {
    ...
    Bank4.setCustomerInfo(_cust_id, _cust_name, _cust_type, {
        from: Bank4_admin,
        gas: 6000000,
        privateFor: [
            key_Bank1,
        ...
        if (!err) {
            ...
            var theEvent = Bank4.EventForSetCustomerInfo({
                from: Bank4_admin,
            });
...
function autotest2() {
    ...
    var frombank = "Bank4";
    var fromcustomer = "004-000-00001";
    var tobank = "Bank4";
    var tocustomer = "004-000-00002";
    var amount = TEST_AMOUNT;
    var securities_id = "A07101";
    var payment = TEST_AMOUNT;
    ...
    var nonce = Math.floor((Math.random() * 100));
    var ts = Math.round(new Date().getTime()/1000);
    var txnseq_1 = "Bank4S"+ fromcustomer+ ts + nonce;
    var txnseq_2 = "Bank4B"+ fromcustomer+ ts + nonce;
